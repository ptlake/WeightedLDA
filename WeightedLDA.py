...
class WeightedLDA(ClassifierMixin, TransformerMixin, BaseEstimator):
    def __init__(self, *, solver='svd', n_components=None, tol=0.0001, store_covariance=False):
        self.solver = solver
        if solver != 'svd':
    ...
    def fit(self, X, y, wgts=None, requires_y=True):
        ...
        rank = np.sum((s > self.tol).astype(np.int32))
        ...
        self.intercept_ = -0.5 * np.sum(coef**2, axis=1) - np.dot(self.xbar_, self.coef_.T) + np.log(self.priors_)
